...
def data_reader_dhcp(data_list, active=False):
    res = []
...
if __name__ == '__main__':
    ...
    dhcp = data_reader_dhcp(list_of_datas, active=True)
    db_exists = os.path.exists('dhcp_snooping.db')
    con = create_connection('dhcp_snooping.db')
    
    if db_exists:
        query_for_dhcp = 'insert into dhcp (mac, ip, vlan, interface, switch, active) values (?, ?, ?, ?, ?, ?)'
